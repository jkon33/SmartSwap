...
export default function PriceTicker({
  ...
  return (
    <div className="w-full bg-[#05060A]/95 backdrop-blur-md text-white py-2 px-4 border-b border-cyan-500/20 select-none relative z-30 shadow-[0_2px_15px_rgba(0,0,0,0.8)]">
      <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 text-xs">
        ...
        <div className="flex items-center space-x-2 shrink-0">
          <div className="flex items-center space-x-1.5 px-2 py-0.5 rounded-full bg-[#0A0E1A] border border-cyan-500/30">
            <span className="relative flex h-2 w-2">
              {connected ? (
                ...
                <span className="relative inline-flex rounded-full h-2 w-2 bg-[#FF0055] shadow-[0_0_8px_#FF0055]"></span>
              )}
            </span>
            <span className="font-mono text-[10px] tracking-widest font-bold uppercase text-cyan-400">
              {connected ? "LIVE ORACLE" : "SYNCING..."}
            </span>
          </div>
          {connected ? (
            <Wifi className="h-3.5 w-3.5 text-[#00F0FF] drop-shadow-[0_0_5px_#00F0FF]" />
          ) : (
            <WifiOff className="h-3.5 w-3.5 text-[#FF0055]" />
          )}
        </div>

        {/* Running ticker bar with neon glow */}
        <div className="flex flex-1 items-center justify-start lg:justify-center overflow-x-auto whitespace-nowrap scrollbar-none gap-6 px-3">
          {displayPairs.map((pair) => {
            ...
            return (
              <div key={pair.key} className="flex items-center space-x-2 shrink-0 group hover:opacity-100 transition-opacity">
                <span className="font-orbitron font-semibold text-[11px] text-cyan-300/80 tracking-wide">
                  {pair.label}
                </span>
                <span className={`font-mono font-bold text-xs ${flashClass}`}>
                  {rate !== null
                    ? pair.isCrypto
                ...
                {/* Trend tiny icons */}
                {trend === "up" && <TrendingUp className="h-3.5 w-3.5 text-[#39FF14] animate-bounce drop-shadow-[0_0_6px_#39FF14]" />}
                {trend === "down" && <TrendingDown className="h-3.5 w-3.5 text-[#FF0055] animate-bounce drop-shadow-[0_0_6px_#FF0055]" />}
              </div>
            );
          })}
        </div>
        ...
        {onToggleScanlines && (
          <div className="shrink-0 hidden sm:flex items-center">
            <button
              ...
              className={`flex items-center gap-1.5 px-2 py-1 rounded text-[10px] font-mono font-bold uppercase transition-all ${
                scanlinesEnabled
