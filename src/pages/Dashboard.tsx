...
export default function Dashboard() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-3 sm:px-6 lg:px-8 py-4 sm:py-8 space-y-6 sm:space-y-8 animate-fade-in relative z-10">
      
      {/* 1. Futuristic Header Overview Banner with Corner Accents */}
      <div className="cyber-card rounded-2xl sm:rounded-3xl p-5 sm:p-8 relative overflow-hidden">
        ...
        <div className="absolute top-0 right-0 -translate-y-12 translate-x-12 h-48 sm:h-64 w-48 sm:w-64 rounded-full bg-cyan-500/20 blur-3xl pointer-events-none" />

        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-5 sm:gap-6 relative z-10">
          <div className="space-y-1.5 sm:space-y-2">
            <div className="flex items-center space-x-2">
              <span className="p-1 sm:p-1.5 rounded-lg bg-cyan-950/80 border border-cyan-400/40 text-cyan-400 shadow-[0_0_8px_rgba(0,240,255,0.4)]">
                <Briefcase className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
              </span>
              <span className="text-[10px] sm:text-xs font-mono font-bold text-cyan-400 tracking-widest uppercase">
                ACTIVE SIMULATED NET-WORTH
              </span>
            </div>
            
            <h1 className="font-orbitron text-2xl sm:text-4xl lg:text-5xl font-black tracking-tight text-white glow-text-cyan">
              ${roundedNetWorth.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </h1>
            
            <p className="text-[11px] sm:text-xs text-slate-300 font-mono flex items-center gap-1.5 sm:gap-2">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            <button
              ...
              disabled={reloading}
              className="px-3.5 sm:px-4 py-2 sm:py-2.5 rounded-xl border border-cyan-500/40 bg-[#0A0E1A]/80 hover:bg-cyan-500/20 text-cyan-300 font-mono font-bold text-xs flex items-center space-x-2 transition shadow-[0_0_10px_rgba(0,240,255,0.2)]"
            ...
            <Link
              to="/swap"
              className="btn-neon-cyan rounded-xl px-4 sm:px-5 py-2 sm:py-2.5 font-orbitron font-bold text-xs flex items-center space-x-2"
            >
            ...
            <Link
              to="/profile"
              className="p-2 sm:p-2.5 rounded-xl border border-cyan-500/30 bg-[#0A0E1A]/80 hover:bg-cyan-500/20 text-slate-300 hover:text-cyan-300 transition"
      ...
      <div className="space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2.5">
          <div className="flex items-center space-x-2">
            <Activity className="h-4 w-4 sm:h-5 sm:w-5 text-cyan-400 drop-shadow-[0_0_6px_#00F0FF]" />
            <h2 className="font-orbitron text-base sm:text-lg font-black tracking-wide text-white">
              CYBER TRADING PANEL
            </h2>
          </div>

          {/* Pair selector pills */}
          <div className="flex flex-wrap items-center gap-1 sm:gap-1.5 p-1 rounded-xl bg-[#0A0E1A] border border-cyan-500/30">
            {["BTC/USD", "ETH/USD", "SOL/USD", "USDT/USD"].map((pair) => (
              <button
                key={pair}
                onClick={() => setSelectedChartPair(pair)}
                className={`px-2 sm:px-2.5 py-1 text-[11px] sm:text-xs font-mono font-bold rounded-lg transition-all ${
      ...
      {/* 3. List of Individual Holdings */}
      <div className="space-y-3 sm:space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="font-orbitron text-base sm:text-lg font-black tracking-wide text-white flex items-center space-x-2">
            <Wallet2 className="h-4 w-4 sm:h-5 sm:w-5 text-cyan-400" />
            <span>PORTFOLIO ASSETS</span>
          </h2>
          <span className="font-mono text-[11px] sm:text-xs text-cyan-400/80 px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-lg bg-cyan-950/40 border border-cyan-500/30">
            {assetsSchema.length} ASSETS
          </span>
        </div>
        ...
        <div className="cyber-card rounded-2xl overflow-hidden">
          ...
          <div className="md:hidden divide-y divide-cyan-500/15">
            {assetsSchema.map((asset) => {
              ...
              return (
                <div key={`m-asset-${asset.code}`} className="p-3.5 bg-[#0A0E1A]/40 flex items-center justify-between gap-3">
                  <div className="flex items-center space-x-3">
                    <div className={`w-9 h-9 rounded-xl border flex items-center justify-center font-orbitron font-black text-xs shrink-0 shadow-[0_0_8px_rgba(0,240,255,0.2)] ${asset.iconBorder || "border-cyan-400/40 text-cyan-300 bg-cyan-950/40"}`}>
                      {asset.code}
                    </div>
                    <div>
                      <p className="font-orbitron text-xs sm:text-sm font-bold text-white">{asset.name}</p>
                  ...
                  <div className="text-right">
                    ...
                    <p className="font-orbitron font-bold text-xs text-[#00F0FF]">
          ...
          {/* Desktop & iPad Holdings Table (>= 768px) */}
      ...
      <div className="space-y-3 sm:space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="flex items-center space-x-1.5 border border-cyan-500/30 rounded-xl bg-cyan-950/30 px-2.5 sm:px-3 py-0.5 sm:py-1 text-[11px] sm:text-xs font-mono font-bold text-cyan-300">
            <Coins className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-cyan-400" />
